import numpy as np
from actors import Actor, Boid, Prey, Predator
from obstacles import Obstacle, Circle, Wall
from vectors2d import Vector


class Simulation:
    def __init__(self, window_size=(1, 1), n_prey=10):
        ...
        self.obstacles = []
        self.n_prey = n_prey
        self.prey_settings = {"max_speed": 0.1, "view_distance": 50, "view_angle": np.pi*1.5, "mass": 5000,
                              "color": (255, 255, 0)}
    ...
    def setup(self):
        ...
        x_vals = np.random.uniform(0, self.window_size.x, self.n_prey)
        y_vals = np.random.uniform(0, self.window_size.y, self.n_prey)
        positions = np.column_stack((x_vals, y_vals))
        velocities = np.random.uniform(-1, 1, (self.n_prey, 2))

        # Populate the simulation with new boids
        self.add_n_prey(self.n_prey, positions, velocities)
    ...
    def add_n_prey(self, n, positions, velocities):
        for i in range(n):
            new = Prey(simulation=self,
                       position=positions[i],
                       velocity=velocities[i],
                       max_speed=self.prey_settings["max_speed"],
                       view_distance=self.prey_settings["view_distance"],
                       view_angle=self.prey_settings["view_angle"],
                       mass=self.prey_settings["mass"],
                       color=self.prey_settings["color"])

            self.prey.append(new)
