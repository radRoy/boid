...
class Particle(object):
    ...
    def __init__(self, r_):
        ...
        self.h = 0.  # kernel size (previously self.d2max)
        # physical variables
        self.r_ = r_  # np.array: position vector, with a coordinate (float) for each dimension
        self.v_ = 0.  # velocity_ see drift1(), etc.
        self.e = 0.  # energy
        self.c = 0.  # speed of sound (prob. based on gas equ. from lec.)
        self.rho = 0.  # mass density [mass/area] construct (float!)
        self.v_pred_ = 0.  # temp. var. for integration step
        self.e_pred = 0.  # temp. var. for integration step
        # cell tree related
        # self.parent_leaf = None  # parent leaf(!) cell this p. is in (treebuild)
...
def calcforce():
    # import
    treebuild()
    knn_density()  # treewalk, gather: all p: calculate p.roh
...
def plot_density_pbc_check(ax, x, y, c, cmap, s=10, alpha=.8):
    ...
    ax.scatter(x + 2, y + 2, c=c, cmap=cmap, s=s, alpha=alpha)
...
def main_sph(particles, root, N, n_dim, rmin, rmax, k, N_steps, dur_step):
    positions_t = []
    densities_t = []

    radius = abs(rmin[0] - rmax[0])**2 / N * k * 1.5
    # need knn before drift1
    positions = []
    densities = []
    for p in particles:
        p.knn(particles, k, root, radius)
        p.density_monaghan()
        positions.append(p.r_)
        densities.append(p.rho)
    ...
    """
    # below 2 call possible in same for loop as knn?
    ...
    return particles, positions_t, densities_t

def main_sph_wind(a, root, N, rmin, rmax, k, N_steps, dur_step):
    pass
...
if __name__ == "__main__":
    ...
    N = 1000  # no. of total particles
    n_dim = 2  # no. of total spatial dimensions
    k = 100  # k nearest neighbours that interact
    fps = 60
    ...
    particles, positions_t, densities_t = main_sph(a, root, N, n_dim, rmin, rmax, k, N_steps, dur_step)
